...
@Getter
public class TeamCaptainCommand extends SubCommand {

    private final Core core;

    public TeamCaptainCommand(Core core) {
        this.core = core;
    }
    ...
    public void perform(Player player, String[] args) {
        ...
        if (!role.equalsIgnoreCase("leader") || !role.equalsIgnoreCase("coleader")) {
            player.sendMessage(CC.translate(this.core.getTeamsSettingsConfiguration()
                    .getString("messages.permission-coleader")));
            player.sendMessage(role);
            return;
        }

        if (args.length == 1) {
            player.sendMessage(CC.translate("/team captain <add/remove> <player>"));
            ...
            if (args.length == 2) {
                ...
                if (teamUser.getTeamByPlayer(coleader) != null && teamUser.getTeamByPlayer(player).equalsIgnoreCase(teamName)) {
                    teamUser.setRoleByPlayer(coleader, "captains");
                }
            }
